...
7. Install the realistic demo salon data (this never deletes or overwrites
   existing records). Choose your own demo password; it is read from the
   environment and only a secure hash is stored:

   ```powershell
   $env:DEMO_ADMIN_PASSWORD='choose-a-strong-demo-password'
   php bin/seed.php
   ```

   The seeded sign-in email is `admin@veloraparlour.demo`. Re-running the
   command is safe and does not create duplicates.
...
9. Open `http://localhost:8000/login`. If you choose not to seed, open
   `http://localhost:8000/setup` once to create the Super Admin account.
